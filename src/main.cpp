...
// float velocity =
//     20.0f;  // Default velocity of the actuator (degrees per seconds)
...
// std::function<void(k_api::Base::ActionNotification)> check_for_end_or_abort(
//     bool& finished) {
...
//   std::cout << "Initializing the arm for velocity low-level control example"
//             << std::endl;
//   try {
...
//       // We are printing the data of the moving actuator just for the example
//       // purpose, avoid this in a real-time loop
//       std::cout << data.actuators()[3].current_motor() << "\n";
//     };
...
//                 fmod(commands[i], 360.0f));
//           }
//         }

//         try {
//           base_cyclic->Refresh_callback(base_command, lambda_fct_callback,
//           0);
//         } catch (...) {
//           timeout++;
//         }
...
//       new k_api::RouterClient(transport_real_time, error_callback);
//   transport_real_time->connect("192.168.1.10", PORT_REAL_TIME);
...
//   (milliseconds) create_session_info.set_connection_inactivity_timeout(
//       2000);  // (milliseconds)
...
//   auto session_manager_real_time = new
//   k_api::SessionManager(router_real_time);
//   session_manager_real_time->CreateSession(create_session_info);
//   std::cout << "Sessions created" << std::endl;

//   // Create services
//   auto base = new k_api::Base::BaseClient(router);
//   auto base_cyclic = new
//   k_api::BaseCyclic::BaseCyclicClient(router_real_time);

//   // Example core
//   auto isOk = example_actuator_low_level_velocity_control(base, base_cyclic);
//   if (!isOk) {
//     std::cout << "There has been an unexpected error in "
//                  "example_cyclic_armbase() function."
//               << std::endl;
//   }
...
int main() {
  ...
  auto base_cyclic = new k_api::BaseCyclic::BaseCyclicClient(
      client.low_level_session_->router_.get());

  // does not block
  auto result_async = base_cyclic->RefreshFeedback_async();
  // blocks
  auto readings = result_async.get();
  std::cout
      << "Reading position through base_cyclic->RefreshFeedback_async()\n";
  for (const auto& actuator : readings.actuators()) {
    std::cout << actuator.position() << " ";
  }
  std::cout << std::endl;

  // I think this blocks
  readings = base_cyclic->RefreshFeedback();
  std::cout << "Reading position through base_cyclic->RefreshFeedback()\n";
  for (const auto& actuator : readings.actuators()) {
    std::cout << actuator.position() << " ";
  }
  std::cout << std::endl;
